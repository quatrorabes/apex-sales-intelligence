#!/bin/bash
# DEPLOY: ContactDetail.tsx - make background visible (glass cards + reduce white wash)
# Target: dashboard_v1/src/pages/ContactDetail.tsx
set -euo pipefail

echo "Deploying ContactDetail glass background fix..."
...
echo "Backup written: ${FILE}.backup-${TS}"

cat > "$FILE" <<'TYPESCRIPT_EOF'
...
git add "$FILE"
git commit -m "fix(ContactDetail): reduce white wash so gradient background shows

- Keep Dashboard_v1 scheme but make cards translucent (bg-white/85 + backdrop blur)
- Reduce/adjust large white content panel (bg-white/70, min-h 320)
- Preserves enrichment section mapping + tabs
"
