...
def ensure_schema():
    ...
    # Core scoring & enrichment columns (subset of your original set)
    columns_to_add = [
        ...
        # Persona columns
        ('persona', 'TEXT' if not IS_PRODUCTION else 'VARCHAR(50)'),
        ('persona_confidence', 'INTEGER'),
        ('persona_multiplier', 'REAL' if not IS_PRODUCTION else 'DECIMAL(3,2) DEFAULT 1.00'),
        ('persona_criteria', 'TEXT'),
        ('persona_date', 'TEXT' if not IS_PRODUCTION else 'TIMESTAMP'),
    ]
    ...
    # Activity + signal tables (Phase 2)
    if IS_PRODUCTION:
        cursor.execute("""
    ...
    try:
        ...
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_persona ON contacts(persona)")
    except Exception:
        pass
