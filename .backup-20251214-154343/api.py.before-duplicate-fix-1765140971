...
"""
    ...
    def standardize_enrichment_output(self, raw_text: str) -> str:
        ...
        replacements = [
            ...
            (r'===\s*PERSON RESEARCH\s*(%s!===)', '=== PERSON RESEARCH ==='),
            (r'===\s*COMPANY RESEARCH\s*(%s!===)', '=== COMPANY RESEARCH ==='),
            (r'===\s*SALES INTELLIGENCE\s*(%s!===)', '=== SALES INTELLIGENCE ==='),
            (r'===\s*PERSONALITY ANALYSIS\s*(%s!===)', '=== PERSONALITY ANALYSIS ==='),
        ]
...
def extract_personality_for_outreach(enrichment: str) -> dict:
    ...
    # Extract MBTI
    mbti_match = re.search(r'(%s:MBTI|Type)[:\s]*([IEFSNTJP]{4})', enrichment, re.IGNORECASE)
    if mbti_match:
...
def generate_fallback_outreach(contact, playbook, template, tone, channel):
    ...
    if channel == 'email':
        return {
            ...
            'cta': "Would you be open to a quick 15-minute call this week%s",
...
def generate_fallback_sequence(contact, playbook, tone):
    ...
    return [
        ...
        },
        {
            ...
            'cta': "15 minutes this week%s",
            'send_day': "Day 4"
        },
        {
            'subject': f"Should I close the loop%s",
...
def generate_fallback_call_script(contact, playbook, call_type):
    ...
    return {
        'opener': f"Hi {contact_name}, this is [Your Name] from {our_company}. Did I catch you at an okay time%s",
        ...
        'discovery_questions': [
            "What's your biggest priority this quarter%s",
            "How are you currently handling [relevant process]%s",
            "What's working well%s What could be better%s",
            "If you could wave a magic wand and fix one thing, what would it be%s",
            "What would success look like for you in the next 6 months%s"
        ],
        'talking_points': [
        ...
        'objection_handlers': {
            'no_time': "I totally understand - you're busy. When would be a better time for a quick 10-minute call%s",
            'not_interested': "I appreciate you being direct. Mind if I ask what solution you're currently using%s",
            'send_info': "Happy to send info. What specifically would be most helpful for you to see%s",
            'have_solution': "That's great you have something in place. How's it working for you%s Any gaps%s",
            'no_budget': "Understood. When does your planning cycle start for next year%s"
        },
        'meeting_ask': "Based on what you've shared, I think a quick 15-minute call to show you [specific thing] would be valuable. How does Thursday at 2pm work%s",
...
def get_cadences():
    ...
    cadences = [row for row in cursor.fetchall()]
    conn.close()
        ...
        if existing['status'] == 'active':
            ...
            cursor.execute('''
                ...
                SET status = 'active', current_step = 0, started_at = %s, 
                    next_action_date = CURRENT_DATE, completed_at = NULL
                WHERE id = %s
...
def advance_enrollment(enrollment_id):
    ...
    if next_step >= len(steps):
        ...
        cursor.execute('''
            UPDATE cadence_enrollments 
            SET current_step = %s, next_action_date = date('now', '+' || %s || ' days')
            WHERE id = %s
...
def get_cadence_queue():
    ...
    cursor.execute('''
        ...
        WHERE e.status = 'active'
          AND e.next_action_date <= CURRENT_DATE
...
def get_cadence_stats():
    ...
    cursor.execute('''
        ...
        WHERE status = 'active' AND next_action_date <= CURRENT_DATE
    ''')
...
# Added: December 7, 2025, 12:41 PM PST
# Match ContactDetailPage.tsx expectations exactly
# ============================================================================

# ICP Match - Returns frontend-expected structure
@app.route('/api/contacts/<int:contact_id>/icp-match', methods=['GET'])
def get_icp_match_frontend(contact_id):
    """Frontend-compatible ICP match endpoint"""
    try:
        conn = get_db()
        cursor = conn.cursor()
        ...
        score = contact.get('match_score') or 0
        tier = contact.get('match_tier') or 'UNSCORED'
        ...
        if contact.get('relevance_score', 0) > 70:
            reasons.append("High relevance to services")
        if contact.get('timing_score', 0) > 70:
            reasons.append("Positive timing indicators")
        if contact.get('title') and any(kw in str(contact.get('title', '')).lower() for kw in ['director', 'vp', 'chief', 'head', 'owner']):
            reasons.append("Decision-maker title")
        
        if not reasons:
            reasons = ["Matches basic criteria"]
        ...
        fit_points = [
            {
                ...
                "detail": f"{contact.get('name')} in {contact.get('title', 'senior')} role",
                "impact": "Direct decision maker"
            },
            {
                "type": "painpoint",
                "title": "Company Profile",
                "detail": f"{contact.get('company')} matches target profile",
                "impact": "High solution fit"
            }
        ]
        
        # Check playbook config
        cursor.execute("SELECT COUNT(*) as count FROM user_profile WHERE user_id = %s", ('default',))
        playbook_row = cursor.fetchone()
        playbook_configured = playbook_row and playbook_row.get('count', 0) > 0
        ...
        return jsonify({
            ...
            "whyusfit": {
                "summary": f"{match_level.lower()} fit based on role and company.",
                "points": fit_points
            },
            "playbookconfigured": playbook_configured
        })
...
# Activities - Returns frontend-expected structure
@app.route('/api/contacts/<int:contact_id>/activities', methods=['GET'])
def get_contact_activities_frontend(contact_id):
    ...
    try:
        ...
        cursor.execute("""
            SELECT 
                created_at, enriched_at, last_scored, last_contacted,
        ...
        if contact.get('last_contacted'):
            activities.append({
                "type": "contact",
                "timestamp": contact['last_contacted'],
                "description": f"Contacted ({contact.get('times_contacted', 0)}x total)"
            })
...
# Meeting Prep - Returns frontend-expected structure
@app.route('/api/contacts/<int:contact_id>/meeting-prep', methods=['GET', 'POST'])
def meeting_prep_frontend(contact_id):
    """Frontend-compatible meeting prep endpoint"""
    try:
        conn = get_db()
        cursor = conn.cursor()
        ...
        # POST - generate meeting prep
        cursor.execute("""
            SELECT name, title, company, enrichment_data 
            FROM contacts 
            WHERE id = %s
        """, (contact_id,))
        ...
        meeting_prep = {
            ...
            "talking_points": [
                "Discuss their company's priorities",
...
# Enrollments - For cadence modal
@app.route('/api/contacts/<int:contact_id>/enrollments', methods=['GET'])
def get_contact_enrollments_frontend(contact_id):
    """Get cadence enrollments for a contact"""
    try:
        conn = get_db()
        cursor = conn.cursor()
        
        cursor.execute("""
            SELECT e.*, c.name as cadence_name
            FROM cadence_enrollments e
            LEFT JOIN cadences c ON e.cadence_id = c.id
            WHERE e.contact_id = %s
            ORDER BY e.started_at DESC
        """, (contact_id,))
        ...
        logger.error(f"Enrollments error: {e}")
...
def generate_meeting_prep(contact_id):
    ...
    if not os.getenv('OPENAI_API_KEY'):
        return jsonify({'prep': {
            ...
            'questions_to_ask': [
                "What's your biggest priority right now%s",
                "How are you currently handling this%s",
                "What would success look like%s",
            ],
            'potential_objections': [
                {'objection': "We're not looking right now", 'response': "I understand. What would need to change for this to become a priority%s"},
                {'objection': "We already have a solution", 'response': "Great! How's that working for you%s Any gaps%s"},
            ],
