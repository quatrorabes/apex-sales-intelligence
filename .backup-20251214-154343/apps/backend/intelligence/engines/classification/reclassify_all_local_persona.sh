#!/bin/bash
# Apex_TSX_JSON_Extractor_RUNNER.sh
# ===================================
# ZERO EXPERIENCE → 100% JSON FIELDS IN 3 MINUTES
# Sales Angel Apex Intelligence - COMPLETE BEGINNER SETUP
# ROI: Dashboard_v1 JSON schema ready → Pipeline visualization

# 🔥 EXECUTE THIS SCRIPT (Double-click or Terminal → ./run.sh)
echo "🚀 Sales Angel Apex Intelligence JSON Extractor - BEGINNER MODE"
echo "================================================================"

# STEP 1: SETUP ENVIRONMENT (30 seconds)
echo "📦 STEP 1: Installing Node.js packages..."
npm init -y > /dev/null 2>&1 || echo "package.json created"
npm i glob tsx @types/node typescript --save-dev > /dev/null 2>&1
npm i -g tsx > /dev/null 2>&1

echo "✅ Node.js environment ready"

# STEP 2: CREATE EXTRACTOR (10 seconds)
cat > Apex_TSX_JSON_Extractor.ts << 'EOF'
...
chmod +x Apex_TSX_JSON_Extractor.ts

# STEP 3: RUN EXTRACTION (10 seconds)
echo "🔍 STEP 2: Extracting ALL JSON fields from TSX files..."
./Apex_TSX_JSON_Extractor.ts

echo "\n🎉 === COMPLETE ==="
echo "📁 Results: apex_json_exports/Apex_Intelligence_JSON_Schema.json"
echo "🚀 READY: Import to Dashboard_v1 pipeline visualization"
echo "\nNEXT MOVE: Apex JSON schema locked. Deploy Dashboard_v1?"
