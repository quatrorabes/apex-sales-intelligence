...
"""
    ...
    def gpt4_intelligence_layer(self, raw_profile: str, contact: dict) -> str:
        """Stage 2: GPT-4 structuring with MBTI + DISC enhancement"""
        name = contact.get('name') or f"{contact.get('firstname', '')} {contact.get('lastname', '')}".strip()
        title = contact.get('title', '')
        company = contact.get('company', '')
        
        prompt = f"""You are an elite sales intelligence analyst specializing in personality assessment and buyer psychology. Transform this raw research into a polished, actionable sales dossier.
...
YOUR TASK:
1. Structure the content using proper markdown (# H1, ## H2, ### H3, **bold**, bullet points)
2. ENHANCE the personality assessments:
   - Myers-Briggs: Provide a specific type (e.g., ENTJ, ISFP) with confidence level and evidence
   - DISC: Provide primary/secondary styles with percentages and behavioral indicators
   - Include communication do's and don'ts for each
3. Make Pain Points SPECIFIC to their actual role at {company}, not generic
4. Create a PERSONALIZED Recommended Opening Line that references something specific about them
5. Add strategic insights that go BEYOND the raw data
6. If information is missing, make intelligent inferences OR state "Not publicly available"

PERSONALITY ASSESSMENT GUIDELINES:

For Myers-Briggs, consider:
- E/I: Do they seem energized by people or ideas? Public speaker vs researcher?
- S/N: Detail-oriented and practical vs big-picture and strategic?
- T/F: Decision-making based on logic vs values and people impact?
- J/P: Structured and planned vs flexible and adaptable?
...
OUTPUT REQUIREMENTS:
- Use the EXACT section structure from the raw research
- Ensure all headers use proper markdown formatting
- Keep citations [1], [2] as-is
- Be specific, actionable, and sales-focused
- The final output should be 3000-5000 words

Return the complete, polished intelligence profile."""
        ...
        try:
            ...
            response = client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system", 
                        "content": "You are an elite sales intelligence analyst who creates comprehensive, actionable contact dossiers. You specialize in personality assessment (MBTI, DISC) and translating research into sales strategy. Your profiles are detailed, well-structured, and immediately actionable for sales professionals."
                    },
            ...
            logger.info("✅ GPT-4 processing complete")
            return result
