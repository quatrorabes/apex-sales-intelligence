...
export default function ApexIntelligence() {
  ...
  const fetchContacts = async () => {
    setLoading(true);
    try {
      const res = await fetch('${import.meta.env.VITE_API_URL || "http://localhost:8000"}/api/contacts');
          ...
          <div style={{ display: 'flex', gap: 12 }}>
            <button
              onClick={async () => {
                setLoading(true);
                try {
                  const res = await fetch('${import.meta.env.VITE_API_URL || "http://localhost:8000"}/api/contacts/score-all', {
                    method: 'POST'
                  });
                  const data = await res.json();
                  if (data.success) {
