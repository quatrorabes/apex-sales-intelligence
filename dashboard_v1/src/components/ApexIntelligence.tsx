...
export default function ApexIntelligence({ contacts: propContacts }: { contacts?: any[] } = {}) {
    ...
    <div className="space-y-6 p-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-400 mt-1">CRE-focused scoring with MDCP + RSS prioritization</p>
        </div>
        <div className="flex gap-3">
          <button onClick={fetchApexScores} disabled={loading || running}
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-slate-900/60 border border-slate-700 rounded-xl p-4">
          <div className="text-xs font-semibold text-slate-400 mb-1">🔥 Immediate Action</div>
          <div className="text-3xl font-bold text-white">{immediateCount}</div>
          <div className="text-xs text-slate-500 mt-1">Hot CRE leads - contact today</div>
        </div>
        <div className="bg-slate-900/60 border border-slate-700 rounded-xl p-4">
          <div className="text-xs font-semibold text-slate-400 mb-1">⚡ High Priority</div>
          <div className="text-3xl font-bold text-white">{highCount}</div>
          <div className="text-xs text-slate-500 mt-1">Strong prospects - this week</div>
        </div>
        <div className="bg-slate-900/60 border border-slate-700 rounded-xl p-4">
          <div className="text-xs font-semibold text-slate-400 mb-1">📊 Average Score</div>
          <div className="text-3xl font-bold text-white">{avgPriority}</div>
          <div className="text-xs text-slate-500 mt-1">Mean priority across contacts</div>
        </div>
        <div className="bg-slate-900/60 border border-slate-700 rounded-xl p-4">
          <div className="text-xs font-semibold text-slate-400 mb-1">✅ Total Scored</div>
          <div className="text-3xl font-bold text-white">{contacts.length}</div>
          <div className="text-xs text-slate-500 mt-1">Contacts with Apex scores</div>
        </div>
      </div>

      <div className="bg-slate-900/60 border border-slate-800 rounded-xl overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-800 flex items-center justify-between">
          <h2 className="text-lg font-semibold text-white">Priority CRE Contacts</h2>
          <p className="text-xs text-slate-400">Sorted by priority (70% role, 30% data completeness)</p>
        </div>
          ...
          {loading ? (
            ...
            <table className="min-w-full text-sm">
              <thead className="bg-slate-900 border-b border-slate-800">
                <tr>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-slate-400">Contact</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-slate-400">Title & Company</th>
                  <th className="px-4 py-3 text-center text-xs font-semibold text-slate-400">Lifecycle</th>
                  <th className="px-4 py-3 text-center text-xs font-semibold text-slate-400">MDCP</th>
                  <th className="px-4 py-3 text-center text-xs font-semibold text-slate-400">RSS</th>
                  <th className="px-4 py-3 text-center text-xs font-semibold text-slate-400">Priority</th>
                  <th className="px-4 py-3 text-center text-xs font-semibold text-slate-400">Urgency</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-slate-400">Action</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-800">
                {contacts.slice().sort((a, b) => (b.priority_score ?? 0) - (a.priority_score ?? 0)).map((c) => (
                  <tr key={c.id} className="hover:bg-slate-800/60">
                    ...
                    <td className="px-4 py-3">
                      ...
                      <div className="text-xs text-slate-400">{c.company || "No company"}</div>
                    </td>
                    ...
                    <td className="px-4 py-3 text-center">
                      <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium ${getUrgencyColor(c.urgency_level)}`}>
                        {c.urgency_level || "—"}
                      </span>
                    </td>
