...
type MainTabId = 'contacts' | 'apex' | 'cadence' | 'raw';
...
export default function App() {
  ...
  const handleEnrichContact = async (contactId: number) => {
    try {
      const res = await fetch(`http://localhost:8000/api/contacts/${contactId}/enrich`, {
        method: 'POST',
      ...
      if (result.success) {
        ...
        const updatedRes = await fetch(`http://localhost:8000/api/contacts/${contactId}`);
        if (updatedRes.ok) {
  ...
  return (
    <div style={{ padding: '16px 8px 0 8px' }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 16, marginBottom: 24 }}>
        {stats.map((s) => (
          <div key={s.label} style={{ background: 'rgba(15,23,42,0.9)', borderRadius: 12, padding: 16, border: '1px solid rgba(148,163,184,0.35)', boxShadow: '0 6px 20px rgba(0,0,0,0.35)' }}>
            <div style={{ fontSize: 11, textTransform: 'uppercase', letterSpacing: 0.6, color: '#9ca3af', marginBottom: 6 }}>{s.label}</div>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 16, flexWrap: 'wrap', alignItems: 'center' }}>
        <div style={{ flex: 1, minWidth: 260, position: 'relative' }}>
          ...
          <input type="text" placeholder="Search name, company, email, or title..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}
            style={{ width: '100%', paddingLeft: 38, paddingRight: 14, paddingTop: 8, paddingBottom: 8, borderRadius: 8, border: '1px solid rgba(148,163,184,0.4)', background: 'rgba(15,23,42,0.85)', color: '#e5e7eb', fontSize: 14, outline: 'none' }} />
        </div>
        <select value={filterTier} onChange={(e) => setFilterTier(e.target.value)}
          style={{ padding: '8px 12px', borderRadius: 8, border: '1px solid rgba(148,163,184,0.5)', background: 'rgba(15,23,42,0.9)', color: '#e5e7eb', fontSize: 13, cursor: 'pointer' }}>
          <option value="all">All tiers</option>
          <option value="HOT">HOT</option>
          <option value="WARM">WARM</option>
          <option value="COLD">COLD</option>
        </select>
        <button type="button" style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 12px', borderRadius: 8, border: '1px solid rgba(99,102,241,0.6)', background: 'rgba(79,70,229,0.25)', color: '#e5e7eb', fontSize: 13, fontWeight: 500, cursor: 'pointer' }}>
          <Download size={16} /> Export
        </button>
      </div>

      <div style={{ background: 'rgba(15,23,42,0.95)', borderRadius: 14, border: '1px solid rgba(30,64,175,0.7)', boxShadow: '0 20px 45px rgba(15,23,42,0.95)', overflow: 'hidden' }}>
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            ...
            <tbody>
              {sorted.map((c) => {
                ...
                return (
                  <tr key={c.id} onClick={() => onSelectContact(c)}
                    style={{ borderBottom: '1px solid rgba(31,41,55,0.9)', transition: 'background 0.18s', cursor: 'pointer', background: isSelected ? 'rgba(33, 128, 141, 0.15)' : 'transparent' }}
                    onMouseEnter={(e) => { if (!isSelected) e.currentTarget.style.background = 'rgba(30,64,175,0.25)'; }}
                    onMouseLeave={(e) => { e.currentTarget.style.background = isSelected ? 'rgba(33, 128, 141, 0.15)' : 'transparent'; }}>
                    <td style={tdStyle}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        <div style={{ width: 32, height: 32, borderRadius: 999, background: 'linear-gradient(135deg, #6366f1 0%, #4f46e5 40%, #0ea5e9 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: 12, fontWeight: 700 }}>
                          {c.name.split(' ').map((n) => n[0]).join('').substring(0, 3).toUpperCase()}
                        </div>
                        <div>
                    ...
                    <td style={tdStyle}><span style={{ color: '#9ca3af' }}>{c.lifecycle_stage || '-'}</span></td>
                    <td style={tdStyle}><span style={{ color: '#9ca3af' }}>{c.lead_status || '-'}</span></td>
                    <td style={tdStyle}><ScorePill value={c.priority_score} colors={priorityColor} /></td>
                    <td style={tdStyle}>
              ...
              {sorted.length === 0 && (
                <tr><td colSpan={8} style={{ padding: 32, textAlign: 'center', color: '#9ca3af' }}>No contacts match your filters.</td></tr>
              )}
            </tbody>
          </table>
        </div>
      </div>
      <div style={{ marginTop: 16, fontSize: 12, color: '#9ca3af', textAlign: 'right' }}>Showing {sorted.length} of {contacts.length} contacts</div>
    </div>
  );
}
...
function TierBadge({ tier }: { tier?: string }) {
  ...
  return (
    <span style={{ padding: '3px 8px', borderRadius: 999, fontSize: 11, border: `1px solid ${style.border}`, background: style.bg, color: style.color, textTransform: 'uppercase', fontWeight: 600 }}>
      {tier || 'COLD'}
    </span>
  );
}
...
function ScorePill({ value, colors }: { value?: number; colors: { color: string; bg: string } }) {
  return (
    <span style={{ display: 'inline-block', minWidth: 32, padding: '4px 10px', borderRadius: 999, fontSize: 13, textAlign: 'center', background: colors.bg, color: colors.color, fontWeight: 600 }}>
      {value != null ? Math.round(value) : '-'}
    </span>
  );
}

function SortableTh(props: { label: string; field: SortField; sortField: SortField; sortDir: 'asc' | 'desc'; onSort: (field: SortField) => void }) {
  const { label, field, sortField, sortDir, onSort } = props;
  const active = sortField === field;
  return (
    <th onClick={() => onSort(field)} style={{ padding: 12, textAlign: 'left', fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: 0.6, cursor: 'pointer', color: active ? '#e5e7eb' : '#9ca3af', whiteSpace: 'nowrap' }}>
...
const tdStyle: React.CSSProperties = { padding: 12, fontSize: 13, color: '#e5e7eb' };