...
// =============================================================================
// API NORMALIZER - unwrap + snake_case -> UI fields
// =============================================================================
function normalizeContact(api: any): Contact {
  return {
    ...
    linkedin_url: api?.linkedin_url ?? api?.linkedinUrl ?? null,
    last_enriched: api?.enriched_at ?? api?.last_enriched ?? null,

    // Optional/common fields used around the page
    name: api?.name ?? `${api?.first_name ?? ''} ${api?.last_name ?? ''}`.trim(),
    vertical: api?.vertical ?? null,
...
export default function ContactDetailPage() {
  ...
  const handleEnrich = async () => {
    ...
    try {
      ...
      const poll = setInterval(async () => {
        const res = await fetch(`${API_BASE}/api/contacts/${id}/enrichment-status`);
  ...
  const handleDownload = async () => {
    setGenerating(true);
    try {
      const res = await fetch(`${API_BASE}/api/contacts/${id}/generate-persona`, { method: 'POST' });
      if (res.ok) {
        const data = await res.json();
        if (data.files?.pdf_landscape) {
          window.open(`${API_BASE}/api/download?path=${encodeURIComponent(data.files.pdf_landscape)}`, '_blank');
        }
      }
    } catch (e) {
      console.error(e);
