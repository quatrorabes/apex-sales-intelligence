#!/bin/bash
# Fix PostgreSQL compatibility + Add debug logging + Limit to 1 contact

set -e

echo "========================================================================"
echo "ENRICHMENT DEBUG FIX"
echo "========================================================================"

# Step 1: Fix enrichment route with PostgreSQL fixes + debug logging
...
echo "✅ Updated: apps/backend/api/routes/enrichment.py"
echo "   - Fixed PostgreSQL syntax"
echo "   - Limited to 1 contact at a time"
echo "   - Added debug file output"

# Step 2: Fix main.py /api/batch/enrich duplicate endpoint
echo ""
echo "Checking for duplicate /api/batch/enrich endpoints in main.py..."

# Comment out or remove main.py's own /api/batch/enrich if it exists
if grep -q "@app.post(\"/api/batch/enrich\")" apps/backend/main.py; then
    echo "⚠️  Found duplicate /api/batch/enrich in main.py - commenting out..."
    
    # Comment out the duplicate endpoint
    sed -i.bak '/@app.post("\/api\/batch\/enrich")/,/^@app\|^def [a-z_]/s/^/# DISABLED: /' apps/backend/main.py
    
    echo "✅ Disabled duplicate endpoint in main.py"
else
    echo "✅ No duplicate /api/batch/enrich in main.py"
fi
...
git commit -m "fix(enrichment): PostgreSQL compatibility + debug logging + limit 1

FIXES:
- PostgreSQL syntax (not PostSQL)
- Limit batch enrichment to 1 contact for testing
- Add debug file output after Perplexity + OpenAI
- Remove duplicate /api/batch/enrich from main.py
- Better error handling and logging

DEBUG FILES:
- /tmp/apex_debug/contact_{id}_01_engine_raw_result_{timestamp}.txt
- /tmp/apex_debug/contact_{id}_02_after_perplexity_and_openai_{timestamp}.txt
- /tmp/apex_debug/contact_{id}_03_after_parsing_{timestamp}.txt

Resolves: Dashboard not showing contacts, batch limit too high"

git push origin main

echo ""
echo "========================================================================"
echo "✅ ENRICHMENT DEBUG FIX DEPLOYED"
echo "========================================================================"
echo ""
echo "CHANGES:"
echo "  🔧 Fixed PostgreSQL query syntax"
echo "  🔧 Limited batch enrich to 1 contact"
echo "  🔧 Added debug file output"
echo "  🔧 Removed duplicate endpoint from main.py"
echo ""
echo "DEBUG FILES LOCATION:"
echo "  📂 /tmp/apex_debug/contact_{id}_*"
echo ""
echo "After deployment:"
echo "  1. Click 'Enrich All' (will enrich only 1 contact)"
echo "  2. Check Render logs for enrichment flow"
echo "  3. SSH to Render instance to view debug files:"
echo "     render ssh <service-name>"
echo "     cat /tmp/apex_debug/contact_*_02_after_perplexity_and_openai_*.txt"
echo ""
