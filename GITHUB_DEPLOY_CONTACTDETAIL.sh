#!/bin/bash
# GITHUB_DEPLOY_CONTACTDETAIL_FIX.sh
# Apex Sales Intelligence | Production GitHub → Vercel/Render
# Dec 15, 2025 | ContactDetail theme + enrich + parser

set -e

echo "========================================"
echo "APEX ContactDetail Fix → GitHub → Deploy"
echo "========================================"

cd ~/projects/apex/apex-sales-intelligence

# 1. Verify clean state (critical)
echo "📋 Checking git status..."
git status

echo ""
echo "⚠️  CONFIRM: Only ContactDetail.tsx should be modified."
...
# 3. Write new ContactDetail.tsx
echo "✍️  Writing new ContactDetail.tsx..."
cat > dashboard_v1/src/pages/ContactDetail.tsx << 'EOF'
...
git commit -m "fix(dashboard): ContactDetail theme unification + enrich wiring + parser

- Match LandingPage gradient (indigo→purple→pink)
- Wire Enrich button → POST /api/contacts/:id/enrich
- Parse enrichment_data JSON → structured UI sections
- Add loading/error states with theme consistency
...
echo "   - Vercel: Rebuilding dashboard_v1..."
echo "   - Render: Backend already live (no changes)"
echo ""
echo "Monitor:"
echo "  Vercel: https://vercel.com/your-project"
