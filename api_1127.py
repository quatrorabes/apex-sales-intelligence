    ...
    try:
        ...
        if content_type in ['all', 'email']:
            ...
            try:
                ...
                # Add the clean function HERE inside the try block
                def clean_email_content(raw_content: str) -> tuple:
                    """Clean OpenAI email response and extract subject/body"""
                    ...
                    if "Subject:" in content:
                        parts = content.split("Subject:", 1)[1]
                        if "\n\n" in parts:
                            subject_part, body_part = parts.split("\n\n", 1)
                ...
                # Clean the emails before saving
                cleaned_emails = []
                for email in emails:
                    # Check if email has combined content that needs cleaning
                    if 'body' in email and 'Subject:' in email.get('body', ''):
                        subject, body = clean_email_content(email['body'])
                ...
                if len(cleaned_emails) >= 3:
                    ...
                    conn.execute("""
                        ...
                        WHERE id = ?
                    """, (
                        cleaned_emails[0].get('subject', ''), cleaned_emails[0].get('body', ''),
                        cleaned_emails[1].get('subject', ''), cleaned_emails[1].get('body', ''),
                        cleaned_emails[2].get('subject', ''), cleaned_emails[2].get('body', ''),
                        datetime.now().isoformat(),
                        contact_id
                    ))
                    conn.commit()
                    conn.close()
                    logger.info(f"✅ Generated {len(cleaned_emails)} emails")
                    
                results['email'] = {'success': True, 'count': len(cleaned_emails)}
            except Exception as e:
...
def score_contact(contact_id):
    """Score a single contact with MDCP + RSS"""
    try:
        engine = ApexScoringEngine(DB_PATH)
        result = engine.score_contact(contact_id, save_to_db=True)
...
def score_all_contacts():
    ...
    try:
        ...
        cursor.execute("""
            ...
            AND profile_content IS NOT NULL
        """)
        ...
        for contact_id in contact_ids:
            try:
                result = engine.score_contact(contact_id, save_to_db=True)
                results.append(result)
                logger.info(f"✅ Scored contact {contact_id}: Priority {result['priority_score']}")
            except Exception as e:
                logger.error(f"❌ Error scoring contact {contact_id}: {e}")
        ...
        logger.error(f"Bulk scoring error: {e}")
        traceback.print_exc()
        return jsonify({'success': False, 'error': str(e)}), 500
    
@app.route('/api/contacts/scored', methods=['GET'])
def get_scored_contacts():
    """Get all scored contacts for Apex Intelligence"""
    try:
        conn = get_db()
        cursor = conn.cursor()
        
        cursor.execute("""
            SELECT id, name, email, phone, company, job_title, title,
...
def start_cadence():
    ...
    try:
        ...
        cadence_type = data.get('type', 'standard')  # aggressive, standard, nurture
        
        if not contact_id:
            return jsonify({'success': False, 'error': 'contact_id required'}), 400
        
        engine = AutoSequenceEngine(DB_PATH)
...
def save_user_preferences():
    ...
    try:
        ...
        conn.execute("""
            INSERT OR REPLACE INTO user_preferences (
                user_id, products, services, value_propositions,
        ...
        """, (
            ...
            data.get('personal_differentiators', '[]'),
            data.get('company_differentiators', '[]')
        ))
        
        conn.commit()
        conn.close()
        
        logger.info(f"✅ User preferences saved")
